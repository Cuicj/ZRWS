...
const steps = [
  {
    ...
    command: 'cp /root/workspace/ZRWS/code/java/zrws-approval/target/zrws-approval.jar /root/workspace/app.jar && ls -la /root/workspace/app.jar',
    timeout: 30000
  },
  {
...
async function main() {
  console.log('='.repeat(60));
  console.log('  ZRWS v4.0.0 Full Deployment (Resume from Step 5)');
  ...
  try {
    ...
    console.log('\n--- Frontend /TZ check ---');
    try {
      await executeCommand(conn, 'curl -I http://localhost/TZ', 30000);
    } catch (e) {
