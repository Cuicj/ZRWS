...
const steps = [
  {
    ...
    command: 'cp /root/workspace/ZRWS/code/java/zrws-approval/target/zrws-approval.jar /root/workspace/app.jar && ls -lh /root/workspace/app.jar',
    timeout: 30000
  },
  {
...
async function main() {
  console.log('='.repeat(60));
  console.log('  ZRWS v4.1.0 Deployment (Resume from Step 5)');
  ...
  try {
    ...
    console.log('\n--- APP download page check ---');
    try {
      await executeCommand(conn, 'curl -I http://localhost/app-download.html', 30000);
    } catch (e) {
      console.log('APP download page check failed');
    }
