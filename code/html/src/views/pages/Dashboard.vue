...
const loadData = async () => {
  try {
    ...
    const [statsRes, tasksRes, devicesRes] = await Promise.all([
      ...
      getRecentTasks({ limit: 5 }).catch(() => ({ data: { list: [] } })),
      getDeviceList().catch(() => ({ data: { list: [] } }))
    ]);
    ...
    if (tasksRes.data?.list && Array.isArray(tasksRes.data.list)) {
      recentTasks.value = tasksRes.data.list.slice(0, 5).map(t => ({
    ...
    if (devicesRes.data?.list && Array.isArray(devicesRes.data.list)) {
      deviceList.value = devicesRes.data.list.slice(0, 5).map(d => ({
        id: d.deviceId || d.id,
