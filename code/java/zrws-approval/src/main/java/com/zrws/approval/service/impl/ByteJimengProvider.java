...
@Slf4j
@Component
public class ByteJimengProvider implements VideoGenerationService.IVideoProvider {

    private static final String PROVIDER_NAME = "BYTE_JIMENG";
    ...
    public void init() {
        ...
        if (providers != null && providers.containsKey(PROVIDER_NAME)) {
            ...
            log.info("字节即梦视频生成提供商初始化完成");
        } else {
            log.warn("字节即梦视频生成配置未找到，跳过初始化");
        }
    }
    ...
    public String submitGenerationTask(VideoTask task) {
        if (config == null || config.getApiKey() == null || config.getApiKey().startsWith("your-")) {
            throw new RuntimeException("字节即梦API配置无效，请在配置文件中设置正确的 api-key");
        }

        try {
            String baseUrl = config.getBaseUrl() != null ? config.getBaseUrl() : "https://api.jimeng.com";
            ...
            if ("IMAGE_TO_VIDEO".equals(task.getGenerationMode())
                ...
                requestBody.put("image_url", task.getReferenceImageUrl());
            }
            ...
            log.info("字节即梦任务提交成功: taskId={}", taskId);
            return taskId;

        } catch (Exception e) {
            log.error("提交字节即梦视频生成任务失败", e);
    ...
    public VideoGenerationService.ProviderTaskStatus queryTaskStatus(String providerTaskId, VideoTask localTask) {
        ...
        try {
            String baseUrl = config.getBaseUrl() != null ? config.getBaseUrl() : "https://api.jimeng.com";
            String url = baseUrl + "/v1/video/task/" + providerTaskId;
            ...
            log.warn("查询字节即梦任务状态异常: taskId={}", providerTaskId, e);
            return null;
        }
    }
    ...
    public void cancelTask(String providerTaskId) {
        ...
        try {
            String baseUrl = config.getBaseUrl() != null ? config.getBaseUrl() : "https://api.jimeng.com";
            String url = baseUrl + "/v1/video/task/" + providerTaskId + "/cancel";
            ...
            log.info("字节即梦任务已取消: taskId={}", providerTaskId);
        } catch (Exception e) {
            log.warn("取消字节即梦任务失败: taskId={}", providerTaskId, e);
        }
    }
}
