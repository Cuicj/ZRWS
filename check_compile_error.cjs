...
conn.on('ready', () => {
  const cmd = `cd /root/workspace/ZRWS/code/java && mvn clean compile -Dmaven.test.skip=true -pl zrws-approval -am 2>&1 | grep -E "ERROR|error:|cannot find|不兼容|找不到|FAILURE|BUILD" | head -30`;
